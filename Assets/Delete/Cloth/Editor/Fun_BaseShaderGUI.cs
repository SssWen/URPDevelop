...
{
    public abstract class Fun_BaseShaderGUI : ShaderGUI
    {
        #region EnumsAndClasses
        private const string EditorPrefKey = "Fun_BaseShaderGUI:";
        ...
        protected class Styles
        {
            ...
            public static readonly GUIContent LayerInputs = new GUIContent("Layer Inputs",
                "These settings describe the Layer stuff.");
            public static readonly GUIContent SparkleInputs = new GUIContent("Sparkle Inputs",
                "These settings describe the Layer stuff.");
            public static readonly GUIContent RimInputs = new GUIContent("Rim Alpha Inputs",
                "These settings describe the RimColor & alpha stuff.");
            public static readonly GUIContent RimLightInputs = new GUIContent("Rim Light Inputs",
                "These settings describe the RimColor stuff.");    
            public static readonly GUIContent TCPRimLightInputs = new GUIContent("TCP Rim Light Inputs",
                "These settings describe the TCP RimColor stuff.");    
                      
            public static readonly GUIContent SharpChangeLightInputs = new GUIContent("Sharp Change Inputs",
                "These settings describe the Sharp Light Change stuff.");          
            public static readonly GUIContent AnistropicInputs = new GUIContent("Anistropic Inputs",
                "These settings describe the Anistropic stuff.");
            public static readonly GUIContent Anistropic = new GUIContent("Anistropic",
                "These settings describe the Anistropic stuff.");
        }
        ...
        private SavedBool m_SurfaceOptionsFoldout;

        private SavedBool m_SurfaceInputsFoldout;

        private SavedBool m_RimFoldout;

        private SavedBool m_RimLightFoldout;

        private SavedBool m_TCPRimLightFoldout;

        private SavedBool m_SharpLightChangeFoldout;

        private SavedBool m_AnistropicFoldout;

        private SavedBool m_LayerInputsFoldout;

        private SavedBool m_SparkleFoldout;

        private SavedBool m_AdvancedFoldout;
        ...
        public virtual void OnOpenGUI(Material material, MaterialEditor materialEditor)
        {
            ...
            m_RimFoldout = new SavedBool($"{m_HeaderStateKey}.RimFoldout", false);
            m_RimLightFoldout = new SavedBool($"{m_HeaderStateKey}.RimLightFoldout", false);
            m_TCPRimLightFoldout = new SavedBool($"{m_HeaderStateKey}.TCPRimLightFoldout", false);
            m_SharpLightChangeFoldout = new SavedBool($"{m_HeaderStateKey}.SharpLightChangeFoldout", false);
            m_AnistropicFoldout = new SavedBool($"{m_HeaderStateKey}.AnistropicFoldout", false);
            m_LayerInputsFoldout = new SavedBool($"{m_HeaderStateKey}.LayerInputsFoldout", false);
            m_SparkleFoldout = new SavedBool($"{m_HeaderStateKey}.SparkleFoldout", false);            
        ...
        public void ShaderPropertiesGUI(Material material)
        {
            ...
            SetFoldoutState($"{m_HeaderStateKey}.SurfaceOptionsFoldout", m_SurfaceOptionsFoldout.value);
            ...
            SetFoldoutState($"{m_HeaderStateKey}.SurfaceInputsFoldout", m_SurfaceInputsFoldout.value);
            EditorGUILayout.EndFoldoutHeaderGroup();

            // DoRim
            m_RimFoldout.value = EditorGUILayout.BeginFoldoutHeaderGroup(m_RimFoldout.value, Styles.RimInputs);
            if (m_RimFoldout.value)
            {
                DrawRimInputs(material);
                EditorGUILayout.Space();
            }
            SetFoldoutState($"{m_HeaderStateKey}.RimFoldout", m_RimFoldout.value);
            EditorGUILayout.EndFoldoutHeaderGroup();

            // DoRimLight
            m_RimLightFoldout.value = EditorGUILayout.BeginFoldoutHeaderGroup(m_RimLightFoldout.value, Styles.RimLightInputs);
            if (m_RimLightFoldout.value)
            {
                DrawRimLightInputs(material);
                EditorGUILayout.Space();
            }
            SetFoldoutState($"{m_HeaderStateKey}.RimLightFoldout", m_RimLightFoldout.value);
            EditorGUILayout.EndFoldoutHeaderGroup();         

            // DoAnistropic            
            m_AnistropicFoldout.value = EditorGUILayout.BeginFoldoutHeaderGroup(m_AnistropicFoldout.value, Styles.AnistropicInputs);
            if (m_AnistropicFoldout.value)
            {
                DrawAnistropicInputs(material);
                EditorGUILayout.Space();
            }
            SetFoldoutState($"{m_HeaderStateKey}.AnistropicFoldout", m_AnistropicFoldout.value);
            EditorGUILayout.EndFoldoutHeaderGroup();

            // DoSharpLightChange
            m_SharpLightChangeFoldout.value = EditorGUILayout.BeginFoldoutHeaderGroup(m_SharpLightChangeFoldout.value, Styles.SharpChangeLightInputs);
            if (m_SharpLightChangeFoldout.value)
            {
                DoSharpLightChangeInputs(material);
                EditorGUILayout.Space();
            }
            SetFoldoutState($"{m_HeaderStateKey}.SharpLightChangeFoldout", m_SharpLightChangeFoldout.value);
            EditorGUILayout.EndFoldoutHeaderGroup();
 
            // DoTCPRimLight
            m_TCPRimLightFoldout.value = EditorGUILayout.BeginFoldoutHeaderGroup(m_TCPRimLightFoldout.value, Styles.TCPRimLightInputs);
            if (m_TCPRimLightFoldout.value)
            {
                DrawTCPRimLightInputs(material);
                EditorGUILayout.Space();
            }
            SetFoldoutState($"{m_HeaderStateKey}.TCPRimLightFoldout", m_TCPRimLightFoldout.value);
            EditorGUILayout.EndFoldoutHeaderGroup();

            // DoLayer
            m_LayerInputsFoldout.value = EditorGUILayout.BeginFoldoutHeaderGroup(m_LayerInputsFoldout.value, Styles.LayerInputs);
            if (m_LayerInputsFoldout.value)
            {
                DrawLayerInputs(material);
                EditorGUILayout.Space();
            }
            SetFoldoutState($"{m_HeaderStateKey}.LayerInputsFoldout", m_LayerInputsFoldout.value);
            EditorGUILayout.EndFoldoutHeaderGroup();

            // DoSparkle
            m_SparkleFoldout.value = EditorGUILayout.BeginFoldoutHeaderGroup(m_SparkleFoldout.value, Styles.SparkleInputs);
            if (m_SparkleFoldout.value)
            {
                DrawSparkleInputs(material);
                EditorGUILayout.Space();
            }
            SetFoldoutState($"{m_HeaderStateKey}.LayerInputsFoldout", m_LayerInputsFoldout.value);
            EditorGUILayout.EndFoldoutHeaderGroup();
            ...
            SetFoldoutState($"{m_HeaderStateKey}.AdvancedFoldout", m_AdvancedFoldout.value);
            EditorGUILayout.EndFoldoutHeaderGroup();
        ...
        public virtual void DrawLayerInputs(Material material)
        {

        }

        public virtual void DrawSparkleInputs(Material material)
        {

        }

        public virtual void DrawRimInputs(Material material)
        {

        }
        
        public virtual void DrawRimLightInputs(Material material)
        {

        }
        public virtual void DrawTCPRimLightInputs(Material material)
        {

        }
        
        public virtual void DoSharpLightChangeInputs(Material material)
        {

        }   
        public virtual void DoWrapLightInputs(Material material)
        {

        }        
        public virtual void DrawAnistropicInputs(Material material)
        {
            ...
            else
            {
                ...
                switch (blendMode)
                {
                    ...
                    case BlendMode.Multiply:
                        ...
                        break;
                    case BlendMode.AlphaDepth:
                        material.SetInt("_SrcBlend", (int)UnityEngine.Rendering.BlendMode.SrcAlpha);
                        material.SetInt("_DstBlend", (int)UnityEngine.Rendering.BlendMode.OneMinusSrcAlpha);
                        material.DisableKeyword("_ALPHAPREMULTIPLY_ON");
                        material.SetInt("_ZWrite", 1);
                        break;
                    case BlendMode.PremultiplyDepth:
                        material.SetInt("_SrcBlend", (int)UnityEngine.Rendering.BlendMode.One);
                        material.SetInt("_DstBlend", (int)UnityEngine.Rendering.BlendMode.OneMinusSrcAlpha);
                        material.EnableKeyword("_ALPHAPREMULTIPLY_ON");
                        material.SetInt("_ZWrite", 1);
                        break;
                    case BlendMode.AdditiveDepth:
                        material.SetInt("_SrcBlend", (int)UnityEngine.Rendering.BlendMode.SrcAlpha);
                        material.SetInt("_DstBlend", (int)UnityEngine.Rendering.BlendMode.One);
                        material.DisableKeyword("_ALPHAPREMULTIPLY_ON");
                        material.SetInt("_ZWrite", 1);
                        break;
                    case BlendMode.MultiplyDepth:
                        material.SetInt("_SrcBlend", (int)UnityEngine.Rendering.BlendMode.DstColor);
                        material.SetInt("_DstBlend", (int)UnityEngine.Rendering.BlendMode.Zero);
                        material.DisableKeyword("_ALPHAPREMULTIPLY_ON");
                        material.EnableKeyword("_ALPHAMODULATE_ON");
        ...
        private void SetFoldoutState(string name, bool value) // , bool field
        {
            // if (field == value)
            //     return;
